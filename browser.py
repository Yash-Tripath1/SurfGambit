...
class BrowserTab(tk.Frame):
    ...
    def navigate_to(self, url: str, is_history_action=False):
        ...
        # Shutdown game loop if active
        if getattr(self, "game_active", False):
            self.game_active = False
            # Unbind all keyboard hooks
        ...
        # Render custom 2D Offline Arcade Game
        if url == "surfgambit://game":
    ...
    def _on_load_error(self, url: str, err_msg: str):
        ...
        <head>
            <style>
                body {{ font-family: sans-serif; margin: 40px; background-color: #000000; color: #ff5722; text-align: center; }}
                .card {{ background-color: #0d0d0d; border: 1px solid #222; padding: 30px; margin: 20px auto; max-width: 500px; text-align: center; border-radius: 8px; }}
                h1 {{ font-size: 28px; margin-bottom: 10px; color: #ff5722; }}
                p {{ font-size: 14px; line-height: 1.5; color: #888; }}
                .url {{ font-family: monospace; background-color: #111; padding: 6px; border: 1px solid #222; color: #ff5722; }}
                .btn {{ background-color: #00adb5; color: white; padding: 10px 20px; border-radius: 4px; text-decoration: none; font-weight: bold; display: inline-block; margin-top: 20px; }}
            </style>
        </head>
        <body>
...
class SurfGambitApp:
    ...
    def _build_ui(self):
        ...
        # BINDING CLICK to trigger surfgambit://game offline shooter!
        self.alien = RetroAlienInvader(nav_row2, size=24, color="#00adb5", bg="#151515", main_app=self)
        self.alien.pack(side="left", padx=8)
        self.alien.bind("<Button-1>", lambda e: self.trigger_game_navigation())
    ...
    def update_tab_bar(self):
        ...
        for idx, tab in enumerate(self.tabs):
            ...
            if title.startswith("http://"):
                ...
                title = title[8:]
            elif title.startswith("surfgambit://"):
                title = title[13:]
            if len(title) > 16:
                title = title[:13] + "..."
    ...
    def trigger_game_navigation(self):
        tab = self.get_active_tab()
        if tab:
            tab.navigate_to("surfgambit://game")
